...
adj_name_lst <- c("RIDAGEYR", "gender", "education_dichotomous", "marital_status", "race")
...
data_info <- list(
    ...
    "self_rated_1" = list(type = "f", weight_var = "WTMEC2YR", ref = "0"),
    "self_rated_2" = list(type = "f", weight_var = "WTMEC2YR", ref = "0"),
    "self_rated_cat" = list(type = "f", weight_var = "WTMEC2YR", ref = "")
)
...
compare_func <- function(df, comp_name_lst, comp_type_lst){
    ...
    for(i in 1:length(comp_name_lst)){
        if(comp_name_lst[i] %in% colnames(df)){
            if(comp_type_lst[i] == "n"){
                weight_var <- get_weight_var(c("self_rated_cat", comp_name_lst[i]))
                #print(paste(comp_name_lst[i], weight_var))
                f_weight <- as.formula(paste0("~", weight_var))
                df_curr <- df[!is.na(df[, weight_var]),]
                dsg <- svydesign(data = df_curr, id = ~SDMVPSU, strata = ~SDMVSTRA, weights = f_weight, nest = T)
                df_sub <- subset(dsg, inAnalysis == "True")
                f_anvtest <- as.formula(paste0(comp_name_lst[i], "~ self_rated_cat"))
                #t_test <- svyttest(f_anvtest, df_sub)
                model <- svyglm(f_anvtest, design = dsg)
                muStd_all <- compute_meanStd(df_sub, comp_name_lst[i])
                muStd_poor <- compute_meanStd(subset(dsg, self_rated_cat == "Poor"), comp_name_lst[i])
                muStd_fair <- compute_meanStd(subset(dsg, self_rated_cat == "Fair"), comp_name_lst[i])
                muStd_good <- compute_meanStd(subset(dsg, self_rated_cat == "Good"), comp_name_lst[i])
                muStd_vgood <- compute_meanStd(subset(dsg, self_rated_cat == "Very Good"), comp_name_lst[i])
                muStd_excellent <- compute_meanStd(subset(dsg, self_rated_cat == "Excellent"), comp_name_lst[i])
                #var_x <- df[df[, "RA"] <= cutoff, comp_name_lst[i]]
                #var_y <- df[df[, "RA"] > cutoff, comp_name_lst[i]]
                #t_test <- t.test(var_x, var_y)
                pval <- regTermTest(model, ~self_rated_cat)$p[1,1]
                
                char_lst <- c(char_lst, comp_name_lst[i])
                overall_lst <- c(overall_lst, muStd_all)
                poor_lst <- c(poor_lst, muStd_poor)
                fair_lst <- c(fair_lst, muStd_fair)
                good_lst <- c(good_lst, muStd_good)
                vgood_lst <- c(vgood_lst, muStd_vgood)
                excellent_lst <- c(excellent_lst, muStd_excellent)
    ...
    df_ret <- data.frame(Characteristics = char_lst, OverallSample = overall_lst, Poor = poor_lst, Fair = fair_lst, Good = good_lst, VGood = vgood_lst, Excellent = excellent_lst, P.Value = pval_lst, RACutoff = cutoff, SampleSize = nrow(df_all))
    return(df_ret)
}
...
output_name_lst_logist <- c("self_rated_1", "self_rated_2")
...
write.csv(df_cutoff, "cutoff_results_2.csv", row.names = F)
...
#print(df_comb)
write.csv(df_comb, "regression_results_2.csv", row.names = F)

# Missing: sleep_duration, Obesity
comp_name_lst <- c("IS60", "IV60", "M10_midpoint", "M10_cnt", "L5_midpoint", "L5_cnt", "RA")
comp_type_lst <- c("n", "n", "n", "n", "n", "n", "n")
df_compare <- compare_func(df_all, comp_name_lst, comp_type_lst)
write.csv(df_compare, "descriptive_results_2.csv", row.names = F)
